  ...
  return (
    <Card className={`cursor-pointer hover:shadow-md transition-shadow relative ${isSelected ? 'ring-2 ring-yellow-300 scale-105' : ''}`}>
      <CardContent className="p-2">
        <div className="aspect-square rounded-lg overflow-hidden mb-2 relative bg-white p-[10%]">
          <OptimizedImage
        ...
        >
          {isSelected ? (
            <span className="flex items-center justify-center text-xs">
              <CheckCircle className="h-4 w-4 mr-1 text-yellow-600" /> {t("builder.added")}
            </span>
          ) : (
            <span className="text-xs">
              {hasAvailableSlots ? t("builder.add") : t("builder.noSlots")}
            </span>
          )}
        </Button>
      </CardContent>
