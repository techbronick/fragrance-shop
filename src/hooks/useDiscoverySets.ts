...
// Fetch a single discovery set config by ID, with its items
export const useDiscoverySetConfig = (id: string) => {
  return useQuery({
    queryKey: ['discovery-set-config', id],
    queryFn: async () => {
      ...
      if (configError) {
        console.error('Error fetching discovery set config:', configError);
        throw configError;
      }
      ...
      if (itemsError) {
        console.error(`Error fetching items for config ${id}:`, itemsError);
        return { ...config, items: [] };
      }
